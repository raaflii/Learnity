    ...
    <ul role="list" class="flex flex-1 flex-col gap-y-7">
        <li>
            <ul role="list" class="-mx-2 space-y-1">
                ...
                <li>
                    <a href="{{ route('search') }}"
                        class="group flex items-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('search') || request()->routeIs('courses*') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}"
                        :class="{ 'justify-center': {{ $collapsed ? 'true' : 'collapsed' }} }">
                ...
                <li>
                    <a href="{{ route('calendar.index') }}"
                        ...
                        <span
                            :class="{ 'opacity-0 hidden': {{ $collapsed ? 'true' : 'collapsed' }}, 'opacity-100': !
                                    {{ $collapsed ? 'true' : 'collapsed' }} }"
                            class="transition-opacity duration-300">Calendar</span>
                    </a>
                </li>
                <li>
                    <a href="{{ route('my-course') }}"
                        class="group flex items-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('my-course') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}"
                        :class="{ 'justify-center': {{ $collapsed ? 'true' : 'collapsed' }} }">
                        <x-heroicon-o-map class="h-6 w-6 shrink-0" />
                        <span
                            :class="{ 'opacity-0 hidden': {{ $collapsed ? 'true' : 'collapsed' }}, 'opacity-100': !
                                    {{ $collapsed ? 'true' : 'collapsed' }} }"
                            class="transition-opacity duration-300">My Course</span>
                    </a>
                </li>
                <li>
                ...
                <ul role="list" class="-mx-2 space-y-1">
                    <li>
                        <a href="{{ route('profile.edit') }}"
                            class="group flex items-center justify-center gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('profile.edit') ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800' }}">
