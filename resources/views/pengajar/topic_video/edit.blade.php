            ...
            <div class="px-4 py-5 sm:p-6">
                <div class="mb-6">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-medium text-gray-900 dark:text-white">Edit Topic Video</h2>
                ...
                <form method="POST" action="{{ route('pengajar.courses.topics.video.update', [$course, $topic, $video]) }}" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Video Title</label>
                        ...
                        <div>
                            <label for="video_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Video Type</label>
                            <select name="video_type" id="video_type" required
                                ...
                                <option value="local" {{ old('video_type', $video->video_type) == 'local' ? 'selected' : '' }}>Local</option>
                            </select>
                        </div>
                    </div>

                    <div>
                        <label for="video_path" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Video Path / URL</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="mt-1 block w-full rounded-md bg-white py-2 dark:bg-gray-700 text-gray-900 dark:text-white ring-1 ring-inset pl-3 ring-gray-300 dark:ring-gray-600 focus:ring-2 focus:ring-blue-600 sm:text-sm">{{ old('description', $video->description) }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            <label for="duration_seconds" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Duration (in seconds)</label>
                        ...
                        <div>
                            <label for="order_index" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Order Index</label>
                            <input type="number" name="order_index" id="order_index" min="0"
                                value="{{ old('order_index', $video->order_index) }}"
                                class="mt-1 block w-full rounded-md py-2 pl-3 bg-white dark:bg-gray-700 text-gray-900 dark:text-white ring-1 ring-inset ring-gray-300 dark:ring-gray-600 focus:ring-2 focus:ring-blue-600 sm:text-sm">
                        </div>
                    </div>
